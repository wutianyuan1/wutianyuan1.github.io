---
permalink: /
title: "About Me"
author_profile: true
lang: en-US
alternate_lang: zh-CN
alternate_url: /zh/
redirect_from: 
  - /about/
  - /about.html
---

I am a Ph.D. candidate in Computer Science and Engineering at [The Hong Kong University of Science and Technology (HKUST)](https://hkust.edu.hk/). I am fortunate to be advised by [Prof. Wei Wang](https://www.cse.ust.hk/~weiwa/).

My research is centered on machine learning systems. My primary focus is on building reliable frameworks for large-scale training. Concurrently, I am exploring reinforcement learning and agentic systems. Prior to my Ph.D., I received my B.E. from the [School of Information Science and Technology](https://sist.shanghaitech.edu.cn/sist_en/) at [ShanghaiTech University](https://www.shanghaitech.edu.cn/eng/) in 2021. I was then an intern at [Microsoft Research Asia](https://www.microsoft.com/en-us/research/lab/microsoft-research-asia/)'s systems group, where I received the "Stars of the Future" award. I received the NSDI'26 Best Paper Award. I have published several research papers on top-tier conferences like OSDI, NSDI, and USENIX ATC.

I strive to live beyond the realm of code and systems, to experience each day authentically, not merely to function. The profound silence felt while hiking the Tibetan plateau, the emotional resonance of [poignant rhythms](https://wutianyuan1.github.io/markdown/), are invaluable reminders that life, in its ephemeral and sometimes melancholic beauty, is defined not by its destination, but by the richness of the journey itself.

Email: twubt [at] connect [dot] ust [dot] hk

{% assign recent_posts = site.categories.technical %}
<div class="home-columns{% unless recent_posts and recent_posts.size > 0 %} home-columns-single{% endunless %}" markdown="1">
<section class="home-column" markdown="1">

NEWS
======
- [09/2026] [CPA]({{ '/files/NeurIPS26_CPA.pdf' | relative_url }}) is accepted to [NeurIPS'26](https://neurips.cc/Conferences/2026) as a poster.
- [09/2026] [AgentProf](http://wutianyuan1.github.io/files/agenticos26-paper27.pdf) will appear at AgenticOS'26.
- [09/2026] [Moye](https://wutianyuan1.github.io/files/sc26-echo.pdf) will appear at [SC'26](https://sc26.supercomputing.org/).
- [05/2026] [Crab](https://arxiv.org/abs/2604.28138) is now accessible on arXiv.
- [05/2026] [PipeMorph](http://wutianyuan1.github.io/files/2504.19232v1.pdf) is honored with the Best Paper Award at [NSDI'26](https://www.usenix.org/conference/nsdi26/).
- [04/2026] Three papers ([Weave](https://arxiv.org/abs/2512.11306v1), [RollArt](https://arxiv.org/abs/2512.22560), and [ASI Trace Analysis](https://www.usenix.org/conference/osdi26)) are accepted to [OSDI'26](https://www.usenix.org/conference/osdi26).
- [01/2026] [Catur](https://openreview.net/forum?id=guCUThRvX5) is accepted to [MLSys'26](https://mlsys.org/Conferences/2026).
- [12/2025] [Weave](https://arxiv.org/abs/2512.11306v1) and [RollArt](https://arxiv.org/abs/2512.22560) are now accessible on arXiv.
- [12/2025] [RollPacker](https://arxiv.org/abs/2509.21009) is accepted to [NSDI'26](https://www.usenix.org/conference/nsdi26/).
- [11/2025] Looking for a ticket for White Album 2 concert held in Yokohama, Japan on Feb 22nd, 2026.
- [11/2025] Three RL-related papers [ROLL](https://arxiv.org/abs/2506.06122), [RollPacker](https://arxiv.org/abs/2509.21009), and [AReaL-Hex](https://arxiv.org/abs/2511.00796) are now accessible on arXiv.
- [07/2025] [PipeMorph](http://wutianyuan1.github.io/files/2504.19232v1.pdf) is accepted to [NSDI'26](https://www.usenix.org/conference/nsdi26/).
- [04/2025] Two papers ([Greyhound](http://wutianyuan1.github.io/files/atc25-wu-tianyuan.pdf) and [Toppings](http://wutianyuan1.github.io/files/atc25-li-suyi-toppings.pdf)) are accepted to [USENIX ATC'25](https://www.usenix.org/conference/atc25).
- [11/2024] [RASTER](http://wutianyuan1.github.io/files/socc24-final138.pdf) is accepted to [ACM SoCC'24](https://acmsocc.org/2024/schedule.html).
- [06/2024] [Portus](https://www.computer.org/csdl/proceedings-article/icdcs/2024/860500a059/1ZCgEAXHPCE) is accepted to [ICDCS'24](https://icdcs2024.icdcs.org/).

</section>
{% if recent_posts and recent_posts.size > 0 %}
<section class="home-column home-posts" markdown="1">

RECENT POSTS
======

<ul class="home-post-list">
{% for post in recent_posts limit: 5 %}
  <li>
    <a class="home-post-title" href="{{ post.url | relative_url }}">{{ post.title }}</a>
    <time datetime="{{ post.date | date_to_xmlschema }}">{{ post.date | date: "%Y-%m-%d" }}</time>
    {% if post.excerpt %}<p>{{ post.excerpt | strip_html | strip_newlines | truncate: 120 }}</p>{% endif %}
  </li>
{% endfor %}
</ul>

[View all posts →]({{ '/posts/' | relative_url }}){: .home-posts-more }

</section>
{% endif %}
</div>
